...
export default function EmergencyFund() {
  ...
  const fmt = (val: number) => formatCurrency(Math.round(val), userData.country);
            ...
            <div className="flex justify-center py-4">
              <div className="relative w-40 h-40">
                <svg viewBox="0 0 120 120" className="w-full h-full -rotate-90">
                  ...
                  <circle cx="60" cy="60" r="52" fill="none" stroke={isFullyCovered ? "hsl(var(--chart-1))" : "hsl(var(--chart-3))"} strokeWidth="10" strokeLinecap="round" strokeDasharray={`${2 * Math.PI * 52}`} strokeDashoffset={`${2 * Math.PI * 52 * (1 - progress / 100)}`} className="transition-all duration-700" />
                </svg>
            ...
            <div className="text-center">
              {isFullyCovered ? (
                ...
                <div className="flex items-center justify-center gap-1.5 text-accent">
                  ...
                  <span className="text-sm font-medium">{coverageMonths < 3 ? "Critically low coverage" : "Building up — keep going!"}</span>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card className="shadow-sm">
            <CardContent className="p-3.5 text-center">
              ...
              <p className="text-lg font-bold tabular-nums">{fmt(requiredFund)}</p>
            </CardContent>
          </Card>
          <Card className="shadow-sm">
            <CardContent className="p-3.5 text-center">
              ...
              <p className="text-lg font-bold tabular-nums text-primary">{fmt(currentFund)}</p>
            </CardContent>
          </Card>
        </div>
            ...
            <Card className="shadow-sm border-accent/20 bg-accent/5">
              <CardContent className="p-4 space-y-2">
                <div className="flex justify-between items-baseline">
                  ...
                  <p className="text-lg font-bold tabular-nums text-accent">{fmt(gap)}</p>
                </div>
                <div className="h-2 bg-secondary rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: `${progress}%`, backgroundColor: "hsl(var(--chart-1))" }} />
                </div>
                <p className="text-xs text-muted-foreground">{fmt(currentFund)} saved of {fmt(requiredFund)} target</p>
              </CardContent>
            </Card>
                ...
                {surplus > 0 ? (
                  <div className="space-y-1.5">
                    <p className="text-sm text-foreground/80">
                      With your monthly surplus of <span className="font-semibold text-primary">{fmt(surplus)}</span>,
                      you can fill the gap in <span className="font-semibold">{monthsToFill} months</span>.
                    </p>
                    <p className="text-xs text-muted-foreground">Save {fmt(surplus)}/month → fully funded by month {monthsToFill}</p>
                  </div>
                ) : (
                  <p className="text-sm text-foreground/80">Your expenses currently exceed your income. Focus on reducing expenses or increasing income first.</p>
                )}
              </CardContent>
            </Card>
          </>
        ...
        <Card className="shadow-sm bg-primary/5 border-primary/10">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              ...
              Keep this in a high-yield savings account or liquid fund.
            </p>
          </CardContent>
        </Card>
      </div>
